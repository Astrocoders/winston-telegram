...
module.exports = class Telegram extends Transport {
  ...
  log(info, callback) {
    ...
    if (this.batchingDelay) {
      ...
      self.send(messageText).finally(() => callback(null, true))
    }
  }
   ...
   * @private
   * @returns {Promise} Promise with send message resolved.
   */
  send(messageText) {
    const self = this

    if (messageText.length < MAX_MESSAGE_LENGTH) {
      return self.sendMessage(messageText)
    } else {
      ...
      return Promise.all(arr.map((message) => self.sendMessage(message)))
    }
  }
    ...
    return new Promise((resolve, reject) => {
      self.request(options, requestData, function (error, response, body) {
        ...
        if (response && response.statusCode !== 200) {
          const errorMessage = response.statusCode + ((body && body.description && ': ' + body.description) || '')
